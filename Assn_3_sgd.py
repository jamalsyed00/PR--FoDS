...
#Target attribute standardization
...
def stoch_grad_des(w,X,y,alpha,error_print,regre,penalty):
  err = []
  for i in range(10):
    for j in range(len(X)):
      ...
      if regre == "noreg":
        ...
        dw = A.dot(B) + (penalty*np.sign(w))*(1/len(X))
        dw[0] = dw[0] - (penalty*np.sign(w[0]))*(1/len(X))
      elif regre == "ridge":
        dw = A.dot(B) + (penalty*2*w)*(1/len(X))
        dw[0] = dw[0] - (penalty*2*w[0])/len(X)
      w = w - (alpha*dw)
  return w,err
...
for i in range(10):
  ...
  print("Polynomial of Degree {}".format(i+1))
  print("Cost Function after 1000 epochs: ")
  weights,error = stoch_grad_des(w,train_X,train_Y,0.0001,error_print = True,regre = "noreg",penalty = 0)
  errors_train.append(cost_function(weights,train_X,train_Y))
  errors_val.append(cost_function(weights,val_X,val_Y))
  errors_test.append(cost_function(weights,test_X,test_Y))
...
for i in range(10):
  ...
  print("Testing data SSE = {}".format(errors_test[i]))
...
plt.xlabel("Degree of Polynomial",fontsize = 13)
plt.ylabel("Validation Error",color = 'orange',fontsize = 13)
...
plt.title("Testing Error v Degree of Polynomial",fontsize = 13)
plt.show()
...
for i in range(10):
  ...
  for pen in hyper:
    ...
    train_X = polynom_feat[i][:train_ratio]
    val_X = polynom_feat[i][train_ratio:train_ratio+valid_ratio]
    weights,error = stoch_grad_des(w,train_X,train_Y,0.0001,error_print = False,regre = "lasso",penalty = pen)
    print("Validation data Cost Function of degree {} and the following hyperparameter {} is {}".format(i+1,pen,cost_function(weights,val_X,val_Y)))
...
for k in range(10):
    ...
    weights,error = stoch_grad_des(w,train_X,train_Y,0.0001,error_print = True,regre = "lasso",penalty = opt_hyper[k])
...
for i in range(10):
  ...
  for pen in hyper:
    ...
    train_X = polynom_feat[i][:train_ratio]
    val_X = polynom_feat[i][train_ratio:train_ratio+valid_ratio]
    weights,error = stoch_grad_des(w,train_X,train_Y,0.0001,error_print = False,regre = "ridge",penalty = pen)
    print("Validation data Cost Function of degree {} and lambda {} is {}".format(i+1,pen,cost_function(weights,val_X,val_Y)))
    val_error.append(cost_function(weights,val_X,val_Y))
    print("")
  val_error = np.array(val_error)  
  ind = np.argmin(val_error)
...
for i in range(10):
    ...
    print("Cost Function after 1000 epochs:")
    weights,error = stoch_grad_des(w,train_X,train_Y,0.0001,error_print = True,regre = "ridge",penalty = opt_hyper[i])
